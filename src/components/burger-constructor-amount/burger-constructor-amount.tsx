...
const BurgerConstructorAmount: FC<IBurgerConstructorTotal> = ({text, icon}) => {
    const [isPopUpOpen, setIsPopUpOpen] = React.useState<boolean>(false);

    const {bun, currentBurgerIngredient} = useSelector(state => state.burgerConstructor);
    const {isLogedIn} = useSelector(state => state.auth);

    const ingridientsIds = (currentBurgerIngredient).map((ingridient) => ingridient._id);
