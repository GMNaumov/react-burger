...
export const OrderCard: FC<IOrderTypes> = ({
  ingredients,
  createdAt,
  name,
  number,
  ...
  const orderIngridients = getOrderIngridients(ingredients, burgerIngredients);
  const allIcons = orderIngridients.map(
    (ingridient) => ingridient.image_mobile
  );
  const sortedIcons = allIcons.filter((x, i) => allIcons.indexOf(x) === i);
  const totalPrice = orderIngridients.reduce(
    (acc: number, ingridient: ICardTypes) => acc + ingridient.price,
    0
  );
  ...
  return (
    ...
    >
      ...
      {visibleStatus && (
        <p className={translatedStatus === "Выполнен" ? styles.success : ""}>
          {translatedStatus}
        </p>
      )}
